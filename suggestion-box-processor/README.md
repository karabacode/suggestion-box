...
All services use the root `requirements.txt`. Build each nested Dockerfile from the repository root, either locally with `docker build -f SERVICE/Dockerfile .` or through its `SERVICE/cloudbuild.yaml`. Because the root is the build context, deploy the built image rather than using `gcloud run deploy --source SERVICE`:

```bash
IMAGE="northamerica-northeast1-docker.pkg.dev/suggestion-box-508020/suggestion-box/suggestion-box:latest"
gcloud builds submit . \
   --config=suggestions-ingester/cloudbuild.yaml \
   --substitutions="_IMAGE=$IMAGE"
gcloud run deploy suggestion-box --image="$IMAGE" --region=northamerica-northeast1
```

Use the matching Cloud Build file and image name for `suggestion-agent` and `suggestion-response-dispatcher`.